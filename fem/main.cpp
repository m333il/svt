...
double source(double x, double y){
    return -a * a * (2.0 * dxy * cos(a * x) * cos(a * y) - (dx + dy) * sin(a * x) * sin(a * y));
}
...
void Problem::assembleLocalSystem(const Cell &c, rMatrix &A_loc, rMatrix &rhs_loc)
{
	...
	for(unsigned i = 0; i < 3; i++) {
		rMatrix grad_i = basis_func_grad(c, nodes[i]);
        for(unsigned j = 0; j < 3; ++j){
			rMatrix grad_j = basis_func_grad(c, nodes[j]);
            A_loc(i, j) = (grad_j.Transpose() * (D * grad_i))(0,0) * c.Volume();
        }
